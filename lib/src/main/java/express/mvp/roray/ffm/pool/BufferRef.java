 ...
 *
 * <p>
 * This is a general-purpose building block for off-heap pooling. It wraps a
 * {@link
 * MemorySegment} with explicit ownership semantics via reference counting,
 ...
 * <li><b>Release:</b> Call {@link #release()} when done; auto-returns to pool
 * at 0
 * </ul>
 *
 ...
 *
 * <p>
 * {@link #poolIndex()} supports SoA patterns where metadata (timestamps, flags,
 * tokens, etc.) is
 * stored in separate primitive arrays indexed by pool index, avoiding
 * additional object overhead.
 *
 * <h2>Thread Safety</h2>
 *
 * <p>
 * Reference counting operations ({@link #retain()}, {@link #release()}) are
 * thread-safe using
 * lock-free CAS operations. The underlying segment access is not synchronized;
 * callers must ensure
...
public interface BufferRef {
     ...
     *
     * <p>
     * <b>Warning:</b> Accessing this segment after {@link #release()} has reduced
     * the refCount to
     * 0 is undefined behavior. The segment may be reused by another thread or the
     * underlying memory
     * may be invalidated.
     */
